...
const AddCustomTurn = (() => { // eslint-disable-line no-unused-vars
  ...
  const handleTurnorderChange = (to,p)=>{
    ...
    if(to.length && to[0].id==='-1') {
      if (to[0].cycleLength !== undefined) {
        to[0].counter++;
        sendChat('API', `/w gm The value of the counter is ${to[0].counter}`);
        sendChat('API', `/w gm The value of the cycle length is ${to[0].cycleLength}`);
        if (to[0].counter == to[0].cycleLength - 1) {
          sendChat('API', `/w gm is this getting seen?`);
          to[0].counter = 0;    
          to[0].pr = 1;
        }
        sendChat('API', `/w gm The value of the pr is ${to[0].pr}`);
        setTurnArray(to);
      }
      ...
      if(msg.content.match(/^!act(\b\s|$)/) ){
        ...
        args.forEach(a=>{
          ...
          switch(parts[0].toLowerCase()){
            ...
            case 'cycle':
              entry.cycleLength = parseInt(parts[1])||3;
              entry.counter = 0;
              break;
